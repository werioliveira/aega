...
module.exports = {
    ...
    LoadRankGuild(){
        return db.promise().query(`SELECT name,level,win FROM player.guild ORDER BY level DESC,win DESC LIMIT 10`);
    },
    OnlineCounter(){
        return db.promise().query("SELECT COUNT(id) as count FROM player.player WHERE DATE_SUB(NOW(), INTERVAL 60 MINUTE) < last_play;")
    },
    ...
    async loadPlayersAll(page){
        ...
        const player = await db.promise().query('SELECT count(*) as numRows FROM player.player')
            if(!player) {
                ...
                const result = await db.promise().query(`SELECT player.player.name as name, player.player.job,player.player.level,player.player.last_play,player.player.exp,player.guild.name as guild,player.player.playtime,player.player_index.empire FROM player.player LEFT JOIN player.guild_member ON player.id = guild_member.pid LEFT JOIN player.guild ON guild_member.guild_id = guild.id INNER JOIN player.player_index ON player.account_id = player.player_index.id WHERE player.name NOT LIKE '[%' ORDER BY player.level DESC , player.playtime DESC LIMIT `+limit)
                    if(!result) {
                        console.log("error: ");
                        return(null);
                    }else{
    ...
    async loadGuilds(){
        ...
        return db.promise().query(`SELECT player.guild.name as guild, player.guild.win,player.guild.ladder_point,player.player.name as player,player.player_index.empire as reino FROM player.guild LEFT JOIN player.player ON player.guild.master = player.id LEFT JOIN player.player_index ON player.player.account_id = player_index.id ORDER BY player.guild.ladder_point DESC limit 20`)  
